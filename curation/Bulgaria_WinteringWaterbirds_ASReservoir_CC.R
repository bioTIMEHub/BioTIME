...
# Dataset: Bulgaria Wintering Waterbirds, Aleksandar Stamboliyski Reservoir
# Location: Bulgaria
# Curator: Cher Chow
...
dt <- read_excel('./Originals/B2SHARE/S011-S019.xlsx', sheet=2, skip=2, col_names=T, na='')
sites <- read_excel('Originals/B2SHARE/S011-S019.xlsx', sheet=11, skip=1, col_names=T, na='')
...
# sp1 because there is consistency that it's a species that cannot be resolved between two
...
dt_merged <- dt %>% group_by_at(vars(-Abundance)) %>% 
  ...
  arrange(Year, Month, Day, Genus, Species)
nrow(dt) - nrow(dt_merged) # any change in aggregating? 0 record
# now create empty columns needed to fit to template
dt_merged$Biomass <- ''
dt_merged$Family <- ''
dt_merged$Plot <- ''
dt_merged$SampleDescription <- as.factor(dt$Year)
length(levels(dt_merged$SampleDescription)) # 26 samples
...
write.csv(dt_merged, 'Bulgaria_WinteringWaterbirds_AStamboliyskiReservoir_rawdata_CC.csv', row.names=F)
