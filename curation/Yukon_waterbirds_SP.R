...
# Import data
raw_data <- read.delim("raw_data/Yukon_waterbirds_raw.csv", sep = ";", dec = ".")
str(raw_data)

# Pivot longer
data <- raw_data %>% pivot_longer(cols = c(2:15),
                                  names_to = "Binomial",
                                  values_to = "Abundance")
...
data$Binomial[data$Binomial == "Anser_canagicus"] <- "Chen_canagica"
data$Binomial[data$Binomial == "Antigone_canadensis"] <- "Grus_canadensis"
...
data$Latitude <- as.numeric(rep(61.11, length = nrow(data)))
data$Longitude <- as.numeric(rep(-165.17, length = nrow(data)))
...
# Save data
write.table(data, "output/Yukon_waterbirds.csv", sep = ";", dec = ".", row.names = FALSE)
