...
rm(list = ls())
# setwd("../")
setwd("./russell-smith_2017_shrubs/")
...
# loading data ----
datafiles <- c(
  "./data/tpsk_shrubs_1994+_p831t1065.csv",
  "./data/tpsl_shrubs_1994+_p831t1123.csv",
  "./data/tpsn_shrubs_1994+_p831t1128.csv"
)
...
ddata[, Abundance := apply(ddata[, .(count_less_than_50cm, `count_50cm-2m`, count_greater_than_2m)], 1, sum, na.rm = TRUE)]
## remove absences
ddata <- ddata[Abundance != 0L]

data.table::setnames(ddata, c("park", "plot","genus_species"), c("Site","Plot","Taxon"))
...
spatial[, Plot := stringi::stri_extract_all_regex(str = plot, pattern = "[0-9]{2,3}")][, Plot := as.integer(sub("^0+(?=[1-9])", "", Plot, perl = TRUE))]
...
dataset_name <- 'russel-smith_2017_shrubs'
