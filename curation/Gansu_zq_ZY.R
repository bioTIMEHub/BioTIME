...
dt_zq <- read_excel('./LZD_SWSJ_Sampling.xls', sheet=1, range=cell_rows(40:51), col_names=T, na='')

colnames(dt_zq)
# "年"             "植物种名"       "拉丁名"         "高度（m）"      "密度（株/㎡ ）"

# 站区调查点荒漠植物群落空间分布格局变化 LZDZQ03 观测场面积50m×50m, 正方形。 
# 中心点坐标: 100°03'49″E, 39°22'23″N。
name_zq <- read_excel('./LZD_SWSJ_Sampling.xls', sheet=1, range=cell_rows(38), col_names=F, na='') %>%
  as.character()
...
lat_zq <- angle2dec("39 22 23") %>% as.numeric()
long_zq <- angle2dec("100 03 49") %>% as.numeric()


## rename column names
# Height (m), Abundance (ind/㎡)
colnames(dt_zq) <- c('Year', 'ChineseName', 'Taxon', 'Height', 'Abundance')


## check year
n_distinct(dt_zq$Year) >= 2


## add Site column

dt_zq$Site <- rep('LZDZQ03A00_01', nrow(dt_zq))

## structure check

dim(dt_zq) # check dimensions, returns row and column counts
summary(dt_zq)
...
is.numeric(dt_zq$Abundance) 

# Year, month and day must be integers or factors
# | means or
is.factor(dt_zq$Year) | is.integer(dt_zq$Year)
...
is.factor(dt_zq$Taxon) | is.character(dt_zq$Taxon)

## fixes
dt_zq$Year <- as.factor(dt_zq$Year)


## remove unnecessary cols

# ChineseName isn't needed
dt_zq$ChineseName <- NULL
# 
dt_zq <- rename(dt_zq, Biomass = Height)

## fields
# Abundance/Density
min(dt_zq$Abundance) > 0 # no zeroes?
sum(dt_zq$Abundance=="") > 0 # no blanks

# Year < 2021, month < 12, day < 31
summary(dt_zq[,1])

## pool abundance
dt_zq %>% group_by(Year, Taxon) %>% 
  summarise(Abundance=sum(Abundance)) %>% ungroup()

dt_zq$Latitude <- rep(lat_zq, nrow(dt_zq))
dt_zq$Longitude <- rep(long_zq, nrow(dt_zq))
...
points <- world + geom_point(data=dt_zq %>% distinct(Latitude, Longitude, Site), 
...
str_which(dt_zq$Taxon, 'idae$|eae$')
...
### prepare and export
dt_zq$Family <- rep('', nrow(dt_zq))
dt_zq$Plot <- rep('', nrow(dt_zq))
dt_zq$DepthElevation <- rep('', nrow(dt_zq))
dt_zq$Day <- rep('', nrow(dt_zq))
dt_zq$Month <- rep('', nrow(dt_zq))
dt_zq$StudyID <- rep('', nrow(dt_zq))


# aggregate abundance records that are same species, plot, and survey day.
df_merged <- dt_zq %>% group_by_at(vars(-Abundance)) %>% 
  summarise(Abundance=sum(Abundance)) %>% ungroup() %>% arrange(Year, Family, Genus, Species)

dim(dt_zq)[1]-dim(df_merged)[1] # any change in aggregating?

## save dataset
# save the dataset name as an object so we save some typing
dataset_name <- 'CERN_Gansu_Linze_desert_plant_communities_LZDZQ03A00_01_2005-2008'
