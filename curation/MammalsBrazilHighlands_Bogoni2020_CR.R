...
bogoni <- read.csv("1. Bogoni et al. (2020) Science of the Total Environment - the mammal dataset 2013-2017.csv")
...
# Assign all Cricetidae a "sp"
bogoni$Species[is.na(bogoni$Species)]<- "sp"
...
bogoni <- bogoni %>% select(Abundance = N.Ind, Biomass, Genus, Species, SampleDescription, 
                            Site, Latitude = y, Longitude = x, Elevation = elevation, Day, Month, Year)
...
write.csv(bogoni, "Bogoni et al. (2020).csv")
