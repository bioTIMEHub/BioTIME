...
coords$latDMS <- gsub("\".*", "", coords$Location)
coords$longDMS <- gsub(".*\ ", "", coords$Location)
coords$longDMS <- gsub("\".*", "", coords$longDMS) # Note: long is in W

# convert to decimal
coords$latDec <- angle2dec(str_replace_all(coords$latDMS, "\\'|\\°", ' '))
coords$longDec <- angle2dec(str_replace_all(coords$longDMS, "\\'|\\°", ' '))
...
sampleTime$FROMDATE <- NULL

dt <- left_join(dt, sampleTime, by = c("SITECODE", "LCODE", "SDATE"))

# Keep only rows where traps were deployed for 1 day
dt <- dt[dt$SPERIOD_D==1,]


# Fixes before going for checks ------------------------------------------------
# Remove fieldnames of Q1 (indicates inability to collect for various reasons)
dt <- dt[!(dt$FIELDNAME=="Q1"),]
...
colnames(dt)[11] <- 'Latitude'
colnames(dt)[12] <- 'Longitude'
...
table(is.na(fieldnameTable$Description)) # 2 are NA
# manual checking reveals they are Fieldname = 30 & 7
# replace the missing fields 
fieldnameTable[fieldnameTable$Fieldname==30, "Description"] <- "Vanessa atalanta"
fieldnameTable[fieldnameTable$Fieldname==7, "Description"] <- "Pieris napi"
# remove everything after the -/- in the Description
fieldnameTable$Taxon <- gsub("\\-.*", "", fieldnameTable$Description)
fieldnameTable$Taxon <- gsub("\\-.*", "", fieldnameTable$Taxon)


sort(unique(fieldnameTable$Taxon))
# Note some species have weird spacings and should actually be -
# Chloroclystis v-ata [884] & Xestia c-nigrum [313] & Lacanobia w-latinum [357]
# replace manually
fieldnameTable[fieldnameTable$Fieldname==313, "Taxon"] <- "Xestia c-nigrum"
fieldnameTable[fieldnameTable$Fieldname==884, "Taxon"] <- "Chloroclystis v-ata"
fieldnameTable[fieldnameTable$Fieldname==357, "Taxon"] <- "Lacanobia w-latinum"


# Separate taxon names - Taxon is 'Genus species'
# Notice some are 'Genus species subspecies' or 'Genus species (morph)'
fieldnameTable$Genus <- word(fieldnameTable$Taxon, 1)
fieldnameTable$Species <- word(fieldnameTable$Taxon, start=2)

# check for mispellings
sort(unique(fieldnameTable$Genus)) # Note Unidentifiable entry
sort(unique(fieldnameTable$Species)) # Note secalis/didyma, species

# Mesapamea secalis was found to be a species complex and all data points after 
# were listed as secalis/didyma. Here, I will make them all secalis/didyma for consistency
fieldnameTable[fieldnameTable$Species=="secalis","Species"] <- "secalis/didyma"
# change species to just be sp
fieldnameTable[fieldnameTable$Species=="species","Species"] <- "sp"

# remove Unidentifiable [2501] entry from dt
dt[dt$FIELDNAME==2501,] <- NA
...
dtALL$SDATE <- NULL
dtALL$SPERIOD_D <- NULL
dtALL$POSIX <- NULL
dtALL$Description <- NULL
dtALL$`Common Name (for species)` <- NULL
dtALL$Taxon <- NULL
...
dim(dt)[1]-dim(dt_merged)[1] # any change in aggregating? yes, 3953
