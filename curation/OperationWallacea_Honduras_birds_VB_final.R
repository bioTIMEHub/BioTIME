...
ds <- read_excel("Honduras birds/BioTIMEContributorTemplate_HonBirdsOPWALL_VB_v02.xlsx", sheet = "rawData")
...
ds <- ds[!is.na(ds$Longitude),]

##remove NA - Joe's error: he left 11 genuses on tail in the Genus column
##                         but Genus can be kept from Genus epiteth in sp column
## map
...
sort(unique(ds$Abundance)) # inspect values - OK!

dim(ds)  #  32295    14


###### asked to check species/genus in previous rounf

##### check taxa ####
sort(unique(ds$Species)) # split between genus and specie (overwrite genus as per Joe said)
...
write.csv(rawdata, paste0(ds_name, "_RAWDATA_VB_v02.csv"), row.names = FALSE)
