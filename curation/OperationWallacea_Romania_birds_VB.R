...
ds <- read.csv("BioTIMEContributorTemplate_RombirdsOPWALL_rawdata.csv")
...
unique(ds$Latitude)
unique(ds$Longitude)
sort(unique(ds$Latitude))
sort(unique(ds$Longitude))

View(ds[ds$Latitude == "TBC",])
## they are charachters. Fix the TBC
#exclude Daia_DA_C09 and Daia_DA_C10
dim(ds)
ds <- ds[ds$Latitude != "TBC",]
dim(ds)

ds$Latitude <- as.numeric(ds$Latitude)
ds$Longitude <- as.numeric(ds$Longitude)
