...
#sub-setting data - we only want years 1997 to 1999

d_new<-d[d$Year > "1995" & d$Year < "2008", ]


unique(d_new$Year)


# check for duplicates within each station, 

sum(duplicated(d$Station[d$Year==1997])) # 0 which means no duplicates
sum(duplicated(d$Station[d$Year==1998]))
sum(duplicated(d$Station[d$Year==1999]))
...
#The variable called Species has genus and species together. 
#We need to create two new variables separating Genus and Species
...
dummy$Family1[str_which(dummy$Genus1,'otea$|aeta$')]<-dummy$Genus1[str_which(dummy$Genus1,'otea$|aeta$')]

unique(dummy$Family1)


# delete the "words" from genus that were copied into Family
dummy$Genus1[dummy$Genus1 == 'Caecidotea'] <- ''
dummy$Genus1[dummy$Genus1 == 'Diporeia'] <- ''
dummy$Genus1[dummy$Genus1 == 'Enchytraeidae'] <- ''
dummy$Genus1[dummy$Genus1 == 'Gastropoda'] <- ''
dummy$Genus1[dummy$Genus1 == 'Lumbriculidae'] <- ''
dummy$Genus1[dummy$Genus1 == 'Naididae'] <- ''
dummy$Genus1[dummy$Genus1 == 'Sphaeriidae'] <- ''
dummy$Genus1[dummy$Genus1 == 'Tubificidae'] <- ''
dummy$Genus1[dummy$Genus1 == 'Chironomidae'] <- ''
dummy$Genus1[dummy$Genus1 == 'Oligochaeta'] <- ''
dummy$Genus1[dummy$Genus1 == 'Orthocladiinae'] <- ''
dummy$Genus1[dummy$Genus1 == 'Tanypodinae'] <- ''
...
dummy$Family1[dummy$Family1==""]<-"NA"
dummy$Species1[dummy$Species1==""]<-"NA"
...
# in 1997 - 
length(unique(d_long$Site[d_long$Year=="1997"]))
#68

# in 1998 - 
length(unique(d_long$Site[d_long$Year=="1998"]))
#114

# in 1999 - 
length(unique(d_long$Site[d_long$Year=="1999"]))
#67
...
write.csv(dt_merged, 'Burlakova_2021_1997&1998&1999_rawdata_MB.csv', row.names=F, na='') # replace your initials here
