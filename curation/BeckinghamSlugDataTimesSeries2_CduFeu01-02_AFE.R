################################################################################
# Study: Slug Numbers - Beckingham 1999 - 2002 (2001-2002) 
# Curator: AFE
...
# Read Data: ===================================================================
files_dir <- "C:/Users/afe1/OneDrive - University of St Andrews/PHD/BIOTIME/BioTIMENewStudiesCuration/BeckinghamSlugDataTimesSeries2_CduFeu01-02_AFE"
dt <- read.csv(file = paste0(files_dir,"/Year3 - Upper-Lower-removal.csv"), h=T, sep=",")


# Structure: ===================================================================
dim(dt)      # 36 x 48
head(dt)     # first sampling April 2001, final February 2002
str(dt)      

names(dt) <- gsub("X","",names(dt))  # rm X in front of date name

dt <- gather(dt, key="Date", value="Abundance", -c(Species, Mat)) # 1656 obs, long format


# Abundance: ===================================================================
sum(is.na(dt$Abundance))        # 1068 are NA's
dt <- dt[!is.na(dt$Abundance),] # 588 (NAs removed)

sum(dt$Abundance <= 0)          # 16 observations are 0's
dt <- dt[dt$Abundance > 0,]     # 572 (0s removed)

range(dt$Abundance)             # 1 to 19


# Sampling Event date: =========================================================
length(unique(dt$Date)) # 46 sampling events (Approx mat checking every two weeks)
                        # This series starts right after the previous one (years 1 & 2) finished
...
unique(dt$Year)
dt$Year <- as.integer(plyr::revalue(dt$Year, c("01"="2001",
                                    "02"="2002")))
...
rawData <- dt %>% group_by(Genus, Species, Latitude, Longitude, Day, Month, Year) %>%
  summarise(Abundance=sum(Abundance)) # pool data from both mats & pool A. circumscriptus & fasciatus together (303 obs)
...
names(rawData)
str(rawData)
range(rawData$Abundance) # 1-31 per sampling event

# Write csv ====================================================================
path <- "C:/Users/afe1/OneDrive - University of St Andrews/PHD/BIOTIMEGithubMetadatas/BeckinghamSlugDataTimesSeries2_CduFeu01-02_AFE"
