...
# Dataset:  Ainsdale Dune Slacks LTE
# Location: England
...
setwd("~/iCloud Drive (Archive)/Documents/BioTIME/Millet")
dt <- as.data.frame(readxl::read_excel('BioTIMEContributorTemplate.xlsx', 
...
sum(dt$Biomass=="") # should have no blanks
...
ggplot(world_map) +
  geom_polygon(aes(x=long, y=lat, group=group), color='grey', fill='transparent') +
  geom_point(data = dt, aes(x = Longitude, y = Latitude), size = 2, shape = 21, colour = 'blue') +
  coord_fixed(ylim = c(53.55, 53.65), xlim = c(-3.09,-3.05)) +
  labs(x = NULL, y = NULL) +
  theme_minimal()
...
write.csv(dt_merged, 'Ainsdale-Dune-Slacks-LTE_rawdata_VB.csv', row.names=F)
library(clipr)
