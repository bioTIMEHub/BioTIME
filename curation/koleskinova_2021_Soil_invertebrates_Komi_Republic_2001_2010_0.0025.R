...
# Dataset: koleskinova_2021_Soil_invertebrates_Komi_Republic_2001_2010_0.0025
...
dataset.name <- 'koleskinova_2021_Soil_invertebrates_Komi_Republic_2001_2010_0.0025sqm'
...
event <- event %>%
  dplyr::filter(sampleSizeValue == 0.0025) %>% 
...
dt <- dt %>% dplyr::filter(occurrenceStatus == 'present') %>%
  ...
  dplyr::mutate(
    ...
    Site = base::gsub(
      pattern = '(<= |^.*buffer-|^.*impact-|^.*background-)(.{3,5})(=?-[0-9]{4}-)([0-9]{1,2})$', 
