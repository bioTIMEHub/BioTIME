...
dt <- readxl::read_excel("TAU_trawl_data.xlsx", 
...
summary(dt$Biomass) # check values
dt$Biomass[dt$Biomass == 0] <- NA
...
ggplot(world_map) +
  geom_polygon(aes(x=long, y=lat, group=group), color='grey', fill='transparent') +
  geom_point(data = dt, aes(x = Longitude, y = Latitude), size = 2, shape = 21, colour = 'blue') +
  coord_fixed(xlim = c(34, 35), ylim = c(31.5,32.2)) +
  labs(x = NULL, y = NULL) +
  theme_minimal()
...
dt$Genus[dt$Genus=="equulites"]<- "Equulites"
dt$Genus[dt$Genus=="Xyrichtys"]<- "Xyrichthys"
dt$Genus[dt$Genus=="sphyraena"]<- "Sphyraena"
dt$Genus[dt$Genus=="conger"]<- "Conger"
...
sort(unique(dt$Species))
dt[dt$Species=="macrophthalma",]
dt[dt$Species=="macrophthalmus",]
dt[dt$Species=="petersi",]
dt[dt$Species=="petersii",]

dt$Species[dt$Species=="sp."] <- "sp"
dt$Species[dt$Species=="petersii"] <- "petersi"
dt$Species[dt$Species=="Torpedo"] <- "torpedo"
...
table(dt$DepthElevation, dt$Longitude)


# aggregate abundance records that are same species and sampling event.
dt$SampleDescription <- as.factor(paste(dt$Latitude,dt$Longitude, dt$DepthElevation, 
                                        dt$Day, dt$Month, dt$Day, sep = "_"))
...
write.csv(dt_merged, 'TAU_trawl_data_rawdata_VB', row.names=F)
library(clipr)
