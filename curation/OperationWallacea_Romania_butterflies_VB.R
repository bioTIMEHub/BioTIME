## Curation script for OpWal: Romanian butterflies
## curator VB

setwd("D:/Dropbox/towards BioTIME v2/Vivi-dataset hunting files/OpWal/BioTIMEv02/BioTIMEv02")
ds_name <- "OperationWallacea_Romania_butterflies"
...
ds <- read.csv("BioTIMEContributorTemplate_RomButOPWALL_rawdata.csv")
...
ds[is.na(ds$Genus),]
unique(ds$Species[is.na(ds$Genus)])
ds$Genus[ds$Species == "Forester Moth sp."] <- "Genus1 sp."
ds$Genus[ds$Species== "Blue sp."] <- "Genus2 sp."
ds$Genus[ds$Species == "Yellow sp."] <- "Genus3 sp."
...
# check that using 1 epiteth doesn't aggregate species
sp_simpl <- c("Genera sp.", "Polygonia c-album","Lycaena rutila",
              "Pseudophilotes vicrama" )
unique(ds[ds$Species %in% sp_simpl,]$Species) # this would work

ds$Species[ds$Species == "Forester Moth sp."] <- "Genera sp."
ds$Species[ds$Species == "Polygonia c album"] <- "Polygonia c-album"
ds$Species[ds$Species == "Lycaena dispar rutila"] <- "Lycaena rutila"
ds$Species[ds$Species == "Pseudophilotes vicrama schiffermuelleri"] <- "Pseudophilotes vicrama"
...
write.csv(unique(dt[duplicated(dt[,2:14]),4:13]), "double_entries_OpWall_Romania_butterflies.csv", row.names = FALSE)
