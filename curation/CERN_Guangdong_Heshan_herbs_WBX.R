# Dataset: CERN_Guangdong_Heshan_herbs
...
setwd("C:/Dropbox/iDiv/Data/BTDataCuration_2023/wubing/CERN Guangdong Heshan_herbs_WBX")
...
dat_plot1 <- read_excel('CERN Guangdong Heshan_herbs_original_WBX/实体数据/鹤山生物数据_草本层植物种组成.xlsx', sheet=2, range = "A2:C28")
dat_plot2 <- read_excel('CERN Guangdong Heshan_herbs_original_WBX/实体数据/鹤山生物数据_草本层植物种组成.xlsx', sheet=3, range = "A2:C25")
...
# species list
species <- read_excel('CERN Guangdong Heshan_herbs_original_WBX/实体数据/鹤山生物数据_动植物名录.xlsx', sheet=3)
...
species_manual <- data.frame(species_cn = species_nomatched$species_cn,
                             species_manual = c("Elephantopus scaber", "Urena lobata", "Cardiospermum halicacabum", "Bolboschoenus maritimus"))
...
# because the precise location of plots in sites are different, I delete the information in column Plot
dat_out$Plot <- ""
...
write_excel_csv(dat_out, file = "CERN_Guangdong_Heshan_herbs_WBX_rawdata.csv")
