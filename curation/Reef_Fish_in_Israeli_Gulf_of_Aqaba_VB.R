...
# Dataset:  Reef Fish in The Israeli Gulf od Aqaba
# Location: Israel
...
setwd("~/iCloud Drive (Archive)/Documents/BioTIME/BelmakerLab/Tal Ga")
dt1 <- readxl::read_excel("Reef fish in the Israeli Gulf of Aqaba (ver 2, different taxonomic resolution).xlsx", 
                          sheet=2, col_names=T, na='')
dt1 <- as.data.frame(dt1[,1:14])

dt2 <- readxl::read_excel("Reef fish in the Israeli Gulf of Aqaba.xlsx", 
                          sheet=2, col_names=T, na='')
...
ggplot(world_map) +
  ...
  coord_fixed(xlim = c(34, 36), ylim = c(29,30)) +
  labs(x = NULL, y = NULL) +
  theme_minimal()
...
write.csv(dt_merged, 'Reef_Fish_in_Israeli_Gulf_of_Aqaba_rawdata_VB.csv', row.names=F)
library(clipr)
