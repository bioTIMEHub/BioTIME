...
setwd("/Users/vivi/Library/CloudStorage/Dropbox/towards BioTIME v2/originalData/v2/MB")
...
# delete variables specimenID, Sex, date & Flower

dt<-dt[,-c(1, 6, 7, 9)]  # delete date
...
dt_merged <- dt %>% group_by(Abundance, Biomass, Family, Genus,
                             Species, Plot,Period, Latitude, Longitude,DepthElevation, Year, Day, Month,StudyID, site) %>%
  summarise(Abundance=sum(Abundance)) %>% ungroup() %>% arrange(Year, Genus, Species)

dt_merged$SampleDescription <-  as.factor(with(dt_merged, paste (Year,Month, Day, Period, Latitude,Longitude, sep='_')))
...
length(levels(dt_merged$SampleDescription)) # how many sampling events? 412
...
write.csv(dt_merged, "Bees and their floral visits in Sapporo in 1979 and 1989_MB_VB.csv") #written csv into working directory
