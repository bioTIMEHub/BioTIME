################################################################################
# Study: Diatom survey in the Northern Range, Trinidad (BioTIME dataset)
# Curator: AFE
# Date: August 2023
################################################################################
...
# Read raw data files ==========================================================
files_dir <- "C:/Users/afe1/OneDrive - University of St Andrews/PHD/BIOTIME/BioTIMENewStudiesCuration/Diatom_T&T_Deacon_et_al_2018_AFE"

load(paste0(files_dir, "/Diatoms.RData"))          # diatom dataset
dt <- as.data.frame(Diatoms)
...
# Abundances ===================================================================
range(dt$abundance)       # 1-3408


# Temporal =====================================================================
sort(unique(dt$timestep)) # 1 to 19
...
# Taxonomy =====================================================================
sort(unique(dt$taxa))  
length(unique(dt$taxa)) # (26) resolution: morphospecies
names(dt)[names(dt)=="taxa"] <- "Family"
sum(is.na(dt$Family))   # 0


# SampleDesc & data aggregation ================================================
rawdata <- dt %>% group_by(Family, site, Latitude, Longitude, Day, Month, Year) %>%
  summarise(Abundance=sum(abundance)) # 4386 obs
...
range(rawdata$Abundance) #  1 3408
rawdata <- within(rawdata, rm(site))

rawdata <- as.data.frame(rawdata)
str(rawdata)

path <- "C:/Users/afe1/OneDrive - University of St Andrews/PHD/BIOTIMEGithubMetadatas/Diatom_T&T_Deacon_et_al_2018_AFE"
