...
# Dataset:  Alcatrazes fish monitoring program
# Location: Brazil
...
setwd("~/iCloud Drive (Archive)/Documents/BioTIME/Ale-Brazil/Alcatrazes_Quimbayo/")
dt <- readxl::read_excel('QuimbayoJP_Alcatrazes_BioTIME.xlsx', 
...
sum(is.na(dt$Family))
dt$Family <- str_to_sentence((dt$Family))

sort(unique(dt$Genus))
summary(as.factor(dt$Genus))
sum(is.na(dt$Genus))
dt$Genus <- str_to_sentence((dt$Genus))
...
length(unique(dt$Plot))
dt %>% 
  distinct(Plot,DepthElevation,Day, Month, Year) %>%
  count(Plot) %>%
  View()

# aggregate abundance records that are same species and sampling event.
dt$SampleDescription <- as.factor(paste(dt$Plot, dt$DepthElevation, dt$Day, dt$Month, dt$Year, sep = "_"))
...
write.csv(dt_merged, 'Alcatrazes_fish_monitoring_program_VB.csv', row.names=F)
library(clipr)
